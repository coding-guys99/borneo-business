...
export default function PublicHeader(){
  ...
  return <header className="topbar global-topbar"><div className="container nav">
    ...
    <nav className="navlinks">{isMember?memberLinks.map(([href,label])=><Link key={href} className={pathname===href?'active':''} href={href}>{label}</Link>):<><Link href="/opportunities">{t('opportunities')}</Link><Link href="/market">{t('market')}</Link><Link href="/procurement-guide">{t('procurementGuide')}</Link></>}</nav>
    <div className="push desktop-actions">{isMember?<><Link className="btn" href="/procurement-guide">{t('procurementGuide')}</Link><button className="btn" onClick={replayGuide}>{t('guide')}</button><button className="btn" onClick={signOut}>{t('signout')}</button></>:<><Link className="btn" href="/signin">{t('signin')}</Link><Link className="btn primary" href="/onboarding">{t('join')}</Link></>}</div>
    ...
    <div className={`mobile-menu ${open?'open':''}`} aria-hidden={!open}>
      {isMember?memberLinks.map(([href,label])=><Link key={href} href={href} onClick={()=>setOpen(false)}>{label}</Link>):<><Link href="/opportunities" onClick={()=>setOpen(false)}>{t('opportunities')}</Link><Link href="/market" onClick={()=>setOpen(false)}>{t('market')}</Link><Link href="/procurement-guide" onClick={()=>setOpen(false)}>{t('procurementGuide')}</Link><Link href="/signin" onClick={()=>setOpen(false)}>{t('signin')}</Link><Link href="/onboarding" onClick={()=>setOpen(false)}>{t('join')}</Link></>}
      {isMember&&<Link href="/procurement-guide" onClick={()=>setOpen(false)}>{t('procurementGuide')}</Link>}
