...
export default function OpportunityPipelineAction({ opportunityId }: { opportunityId: string }){
  ...
  async function save(){setBusy(true);const {data:{user}}=await supabase.auth.getUser();if(!user){router.push(`/signin?next=${encodeURIComponent(`/opportunities/${opportunityId}`)}`);return}
    ...
    if(existing){setMessage('This opportunity is already saved.');setBusy(false);return}
    ...
    setMessage(error?error.message:'Opportunity saved.');setBusy(false)
  }
  return <div className="action-stack"><button className="btn primary" onClick={save} disabled={busy}>{busy?'Saving…':'Save Opportunity'}</button>{message&&<span className="meta">{message}</span>}</div>
}
