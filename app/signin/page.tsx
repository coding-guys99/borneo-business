...
export default function SignInPage(){
  ...
  async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setMsg('Signing in…');const {error}=await supabase.auth.signInWithPassword({email,password});if(error){setMsg(error.message);setBusy(false);return}const next=typeof window!=='undefined'?new URLSearchParams(window.location.search).get('next'):null;router.push(next||'/dashboard');router.refresh()}
